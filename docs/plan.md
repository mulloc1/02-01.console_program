...
│       ├── decorators.py            # 예외/로그/시간 측정 데코레이터
...
1. `@log_command(name)` — CLI 핸들러 진입/완료 로그(stderr).
2. `@translate_errors` — 도메인 예외(`UserInputError`, `NotFoundError`)를 **원인 + 해결 힌트** 메시지로 변환하고 비-0 종료 코드를 유도(subject §4.15).
3. `@measure_time` — 옵션 `-verbose` 시에만 stderr 로 실행 시간 출력.
...
| **stderr** | `[WARN]`, `[ERROR]`, 데코레이터 진단 로그(`@log_command`, `@measure_time -verbose`) |
...
- `src/budget_app/__init__.py`, `__main__.py` 만 둔 빈 패키지 생성.
...
- `decorators.py` (`translate_errors`, `log_command`, `measure_time`).
- `errors.py` (도메인 예외).
