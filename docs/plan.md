...
├── budget_app/
│   ├── __init__.py
│   ├── __main__.py                  # python -m budget_app → main() 위임
│   ├── main.py                      # 진입 조립 · DEFAULT_CATEGORIES·부트스트랩 · 디스패치
│   ├── parser.py                    # argparse 서브커맨드 그래프(-help, -data-dir 등)
│   ├── cli.py                       # 서브커맨드 핸들러 · resolve_command_handler
│   ├── models.py                    # Transaction / Category / Budget (dataclass)
│   ├── types.py                     # 검색 필터·월별 요약 등 공유 DTO (비영속)
...
- `parser.py` + `cli.py` 서브커맨드 구성(단일 `-` 옵션), `__main__.py` 연결.
- `add` 대화형, `list/search/summary/budget/category/update/delete/import/export` 옵션 인자.
- 테스트: `test_cli.py`(`subprocess` 또는 `budget_app.main.main([...])` 직접 호출, stdin/stdout 캡처).
